namespace StudentSystem.Common.Infrastructure.Extensions
{
    ...
    public static class IMemoryCacheExtenstions
    {
        public static void ClearRegionFromCache(this IMemoryCache cache, string regionKey)
        {
            ClearRegionFromCache(cache, regionKey, string.Empty);
        }

        public static void ClearRegionFromCache(this IMemoryCache cache, string regionKey, string collectionKey)
        {
            foreach (var keyToRemove in GetKeys(cache))
            {
                var keyName = keyToRemove?.ToString();

                if (!string.IsNullOrWhiteSpace(keyName) && keyName.StartsWith(regionKey) ||
                    !string.IsNullOrWhiteSpace(collectionKey) && keyName.StartsWith(collectionKey))
                {
